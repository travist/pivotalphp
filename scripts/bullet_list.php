...
function pdf_contents( &$pdf, $args, $stories, &$output ) {
  ...
  foreach ($type_cnt as $the_type => $the_cnt) {
    ...
    $output .="</td>";
    $output .= "<td>";
    $output .= sprintf("%'06d",isset($est_accept_type_cnt[$the_type]) ? $est_accept_type_cnt[$the_type] : 0);
    $output .="</td>";
    $output .= "</tr>";
  }
  ...
  foreach ($type_cnt as $the_type => $the_cnt) {
    ...
    foreach ($states as $thestate) {
      $output .= "<td>";
      $output .= isset($state_type_cnt[$the_type]) && isset($state_type_cnt[$the_type][$thestate]) ? $state_type_cnt[$the_type][$thestate] : '';
      $output .= "</td>";
    }
    $output .= "</tr>";
  }
  ...
  $output .= "<td>".count($stories)."</td>";
   ...
   $tot = 0;
   foreach ($type_cnt as $the_type => $the_cnt) {
     if (isset($state_type_cnt[$the_type][$thestate])) {
    ...
    if (($story['story_type'] != 'release') && ($story['story_type'] != 'chore')) {
     ...
     foreach ($table_col as $col_name) {
       ...
       if ($col_name == "id") {
         ...
         $output .= isset($story[$col_name]) ? $story[$col_name] : ' ';
       }
       $output .= "</td>";
     }
     $output .= "</tr>\n";
